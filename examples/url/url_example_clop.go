...
func (c *Options) Parse(args []string) error {
	...
	// 从环境变量中读取值if envVal := os.Getenv("API_URL"); envVal != "" {
	...
	for i := 0; i < len(args); i++ {
		...
		case arg == "-w,--website":
			...
			if i+1 >= len(args) {
				return fmt.Errorf("missing value for -w,--website")
			}
			
			u, err := url.Parse(args[i+1])
			if err != nil {
				return fmt.Errorf("invalid URL for -w,--website: %v", err)
			}
			c.Website = *u
		...
		case arg == "--api":
			...
			u, err := url.Parse(args[i+1])
			if err != nil {
				return fmt.Errorf("invalid URL for --api: %v", err)
			}
			c.ApiURL = u
